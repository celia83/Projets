...
<section id="boutons">
    ...
    <table id="mini_calendrier" hidden>
        <tr><td colspan="7" align="center"><a href="accueil.php?mois=<?php echo $num_mois-1; ?>&amp;annee=<?php echo $num_an; ?>"><<</a>&nbsp;&nbsp;<?php echo $tab_mois[$num_mois];  ?>&nbsp;&nbsp;<a href="accueil.php?mois=<?php echo $num_mois+1; ?>&amp;annee=<?php echo $num_an; ?>">>></a></td></tr>
        <tr><td colspan="7" align="center"><a href="accueil.php?mois=<?php echo $num_mois; ?>&amp;annee=<?php echo $num_an-1; ?>"><<</a>&nbsp;&nbsp;<?php echo $num_an;  ?>&nbsp;&nbsp;<a href="accueil.php?mois=<?php echo $num_mois; ?>&amp;annee=<?php echo $num_an+1; ?>">>></a></td></tr>
        <?php
        echo'<tr>';
        for($i = 1; $i <= 7; $i++){
...
<section id="zone_reservation">
    ...
    <div id="carte_calendrier">
        <div class="map">
            <input type="image" id="bat" style="width:985px; height:550px" src="<?php
            include_once("connexion2.php");
            $requete = "SELECT alt FROM plan";
            $requete2 = "SELECT salle, shape, coords, image FROM batiment";
            if ($reponse=$bdd2->query($requete)) {
                $enr = $reponse->fetch();
                if ($_GET['c'] == 1) {
                    echo 'Images/BATIMENT_Z.png';
                } elseif ($_GET['c'] == 2) {
                    echo 'Images/HALLNORD.png';
                } elseif ($_GET['c'] == 3) {
                    echo 'Images/BATIMENT_A.png';
                } elseif ($_GET['c'] == 4) {
                    echo 'Images/BATIMENT_E.png';
                } elseif ($_GET['c'] == 5) {
                    echo 'Images/BATIMENT_D.png';
                } elseif ($_GET['c'] == 6) {
                    echo 'Images/BATIMENT_C.png';
                } elseif ($_GET['c'] == 7) {
                    echo 'Images/BATIMENT_B.png';
                } elseif ($_GET['c'] == 8) {
                    echo 'Images/BATIMENT_P.png';
                } else {
                    echo 'Images/BATIMENT_I.png';
                }
            }?>"  usemap="#bat">
            <map name="bat">
                <?php
                $requete = "SELECT shape, coords, image, salle, page FROM batiment";
                if ($reponse=$bdd2->query($requete)) {
                    while ($enr = $reponse->fetch()) {
                        echo "<area shape=" . $enr['shape'] . " coords=" . $enr['coords'] . " href=" . $enr['page'] . " salle=" . $enr['salle'] . "/>" . "\n";
                    }
                }
                ?>
            </map>
